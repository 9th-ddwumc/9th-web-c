...
function useThrottle<T>(value:T, delay =500): T {
    ...
    //2.Ref 변수 lastExcecuted: 마지막으로 실행된 시간을 기록하는 변수
    //useRef 사용하면 컴포넌트가 리랜더링 되어도 값이 유지되고, 변경되어도 리렌더링을 트리거하지 않아요
    //useState 사용하면 리랜더링시 값이 바뀜
    const lastExcecuted = useRef<number>(Date.now());

    //3.useEffect:value, delay가 변경될 때 아래 로직 실행
    useEffect(()=>{
        //현재 시각과 lastExcecuted.current에 저장된 마지막 시각+ delay를 비교합니다.
        //충분한 시간이 지나면 바로 업데이트

        
        if(Date.now() >= lastExcecuted.current + delay){
            //현재 시간이 지난 경우,
            //현재시각으로 lastExcecuted 업데이트
            lastExcecuted.current = Date.now();

            //최신 value를 throttledValue에 저장해서 컴포넌트 리렌더링
            setThrottledValue(value);
        }else{
            //충분한 시간이 지나지 않은 경우, delay 시간 후에 업데이트(최신value로)
            const timerId = setTimeout(()=>{
                //타이머가 만료되면, 마지막 업데이트 시간을 현재 시각으로 갱신합니다.
                lastExcecuted.current = Date.now();
                //최신 value를 throttledValue에 저장해서 커모넌트 리렌더링
                setThrottledValue(value);
            },delay);
