...
onClick={()=>navigate(`/movie/${movie.id}`)}
    ...
    className=""/>
    {isHovered &&(
        <div className='absolute inset-0 bg-gradient-to-t from-black/50 to-transparent bakdrop-blur-md flex-col justify-center items-center text-white p-4'>
        <h2 className='text-lg font-bold text-center leading-snug'>{movie.title}</h2>
        <p className='text-sm text-gray-300 leading-relexed mt-2 line-clamp-5'>{movie.overview}</p>
        </div>
        )}
</div>
);
